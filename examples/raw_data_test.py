    ...
    with radar:
        ...
        def raw_callback(reading):
            ...
            if reading.speed or reading.range_m or reading.magnitude:
                parsed = []
                if reading.speed: parsed.append(f"speed={reading.speed}")
                if reading.range_m: parsed.append(f"range={reading.range_m}")  
                if reading.magnitude: parsed.append(f"mag={reading.magnitude}")
                if reading.direction: parsed.append(f"dir={reading.direction.value}")
                print(f"    ✅ Parsed: {', '.join(parsed)}")
            else:
                print(f"    ⚠️  No data parsed from raw string")
            print()
